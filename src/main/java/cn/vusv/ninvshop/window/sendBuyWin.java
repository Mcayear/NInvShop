...
public class sendBuyWin implements Listener { //一般实际开发中不在这个类中写监听器
    ...
    public sendBuyWin(Player player, ShopPagesData.ItemData itemData, Item slotItem) {
        ...
        if (itemData.getBuyLimits() == null) {
            ...
            form = buyLimitsFrom(new FormWindowCustom("Shop - 批量购买"));
        }
        player.showFormWindow(form, UID);
    }

    public FormWindowCustom buyLimitsFrom(FormWindowCustom form) {
        int limBuyNum = 0;
        ...
        if (slotItem.getCustomName().isEmpty()) {
            label.add("物品名: " + slotItem.getName());
        } else {
            label.add("物品名: " + slotItem.getCustomName());
        }
        if (false) label.add("每份价格: data.price");
        if (!itemData.getNeed().isEmpty()) {
            label.add("每份需求: " + itemData.getNeed());
        ...
        form.addElement(new ElementSlider("购买份数", 0, limBuyNum, limBuyNum > 0 ? 1 : 0, limBuyNum > 0 ? 1 : 0));  // 组件角标: 3
        return form;
    }
